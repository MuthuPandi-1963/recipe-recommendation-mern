...
export default function Navbar() {
  ...
  const handleSearch = async (e) => {
   const  items=e.target.value
    try {
     const res= await fetch(`https://dummyjson.com/recipes/search?q=${items}`)
       setSearch(res.json()||[])
       console.log(search);
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <Link className=" text-white  border-r-0 flex content-center">
              ...
              <Search className="justify-center  rounded-br-sm rounded-tr-sm h-auto pr-2 bg-gray-700" size={23} onChange={handleSearch} />
            </Link>
          </div>
